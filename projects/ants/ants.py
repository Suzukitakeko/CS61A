...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place_curr = self.place
        if distance > 0:
            for _ in range(distance):
                place_curr = place_curr.entrance
                if place_curr == None or place_curr == beehive:
                    return None
        
        while len(place_curr.bees) == 0 and self.min_range <= distance < self.max_range:
            place_curr = place_curr.entrance
            distance += 1
            if place_curr == beehive:
                break
        if len(place_curr.bees) > 0 and place_curr != beehive:
            return  rANTdom_else_none(place_curr.bees)
        else:
            return None
        # END Problem 3 and 4
