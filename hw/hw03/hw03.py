...
def replace_leaf(t, find_value, replace_value):
    ...
    if is_leaf(t):
        if label(t) == find_value:
            ...
            return tree(label(t))
    return tree(label(t), [replace_leaf(tree, find_value, replace_value) for tree in branches(t)])
...
def has_path(t, word):
    ...
    if len(word) == 1:
        if label(t) == word:
            ...
            return False
    if label(t) == word[0]:
        return any([has_path(tree, word[1:]) for tree in t])
    else:
        return False
